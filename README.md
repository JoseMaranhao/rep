...
After installing dependencies, run `python -m apartment_condition_report.web` from this repository and open `http://127.0.0.1:8765` in the same computer's browser. Select a local photo folder, optionally enable AI suggestions, and download a ZIP package containing `report.pdf` and `evidence/`. The interface listens only on your computer.
